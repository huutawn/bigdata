...
Luồng dữ liệu: `generator -> Kafka -> stream-processor -> ml-api -> ClickHouse -> Grafana`
...
- `generator`: thất bại nhanh (fail fast) nếu Kafka không khả dụng.
- `stream-processor`:
  - Nếu Kafka không trả về dữ liệu, hãy tải `raw-logs.sample.jsonl`.
