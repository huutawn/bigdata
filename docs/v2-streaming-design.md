...
Tài liệu này giải thích ý nghĩa của `v2` trong dự án này, cách dữ liệu di chuyển từ log thô đến các dự đoán ML, cách các gói dữ liệu đặc trưng (feature payloads) được xây dựng, và sự khác biệt giữa `NATS batching` (gom nhóm NATS) và `ML windowing` (cửa sổ ML).
...
`sự kiện thô -> trạng thái luồng -> cửa sổ đặc trưng -> gói dữ liệu ML cụ thể theo nhiệm vụ -> dự đoán`
...
1. `generator` đẩy các sự kiện yêu cầu thô lên NATS subject `logs.raw`
2. `stream-processor` đọc một nhóm vận chuyển nhỏ (transport batch) từ NATS
3. luồng dữ liệu chuẩn hóa các sự kiện đó và thêm chúng vào trạng thái trong bộ mới (in-memory state)
4. luồng gom nhóm trạng thái vào các cửa sổ phân tích (analysis windows) và các nhóm dự báo (forecast buckets)
...
## 5. Phân biệt quan trọng nhất: Nhóm NATS (NATS batch) so với Cửa sổ ML (ML window)

Hai ý tưởng này hoàn toàn khác nhau.

### Nhóm NATS (NATS batch)

Một `NATS batch` chỉ là một nhóm vận chuyển nhỏ.
Nó trả lời câu hỏi:

"Chúng ta lấy bao nhiêu tin nhắn từ NATS trong một chu kỳ thăm dò (polling cycle)?"
...
- đăng ký (subscribe) vào `logs.raw`
- thu thập các tin nhắn cho đến khi:
  - nhóm đạt tới `STREAM_BATCH_SIZE`, hoặc
  - hết thời gian thăm dò (timeout)
- sau đó dừng đọc và xử lý bất cứ thứ gì đã thu thập được

Vì vậy, một nhóm NATS là về hiệu quả thu nạp dữ liệu, không phải về ngữ nghĩa ML.
...
- `NATS batch` = nhóm vận chuyển vi mô (transport micro-batch)
...
1. nhận các sự kiện thô từ NATS
...
Spark được sử dụng ở đây như một công cụ xử lý cửa sổ cục bộ khi `STREAM_USE_SPARK_WINDOWS=1`.

Có hai luồng (path) trong bộ xử lý luồng:

- Python window builders (Xây dựng cửa sổ bằng Python)
- Spark-backed window builders (Xây dựng cửa sổ dựa trên Spark)

Spark được sử dụng để:

- gom nhóm các bản ghi thành các cửa sổ thời gian
- tổng hợp theo từng thực thể hoặc từng endpoint
- tính toán số lượng, giá trị trung bình và ước tính phân vị

Lưu ý quan trọng:

Spark không thay thế logic thăm dò NATS (NATS polling) ở đây.
NATS vẫn cung cấp các nhóm thu nạp dữ liệu (ingestion batches).
Spark được sử dụng *sau khi* thu nạp để tính toán các cửa sổ phân tích.

Vì vậy, mối quan hệ là:

- NATS đưa ra nhóm vận chuyển
- luồng lưu trữ trạng thái
- Spark hoặc Python tính toán các cửa sổ đặc trưng
